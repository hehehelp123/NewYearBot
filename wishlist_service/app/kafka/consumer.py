...
async def handle_wishlist_add_event(event_data: dict):
    try:
        ...
        if not url_match:
            logger.warning(f"No URL found in message from {wishlist_request.telegram_id}: {wishlist_request.source_text}")
        ...
        async with AsyncSessionLocal() as session:
            asyncio.create_task(
                WishlistService(session).add_to_wishlist(
                    source_url_str, wishlist_request.telegram_id, is_infinitely_bookable
                )
            )
...
async def handle_wishlist_create_event(event_data: dict):
    try:
        wishlist_create_data = WishlistCreate(
            owner_user_id=event_data["telegram_id"], name=event_data["username"]
        )
        async with AsyncSessionLocal() as session:
            try:
...
async def handle_wishlist_book_event(event_data: dict):
    try:
        book_request = ItemBookRequest(item_id=event_data["item_id"], booker_user_id=event_data["booker_user_id"])
        async with AsyncSessionLocal() as session:
            service = WishlistService(session)
...
async def handle_wishlist_unbook_event(event_data: dict):
    try:
        unbook_request = ItemBookRequest(
            item_id=event_data["item_id"], booker_user_id=event_data["unbooker_user_id"]
        )
        async with AsyncSessionLocal() as session:
            service = WishlistService(session)
...
async def handle_wishlist_delete_event(event_data: dict):
    try:
        delete_request = ItemDeleteRequest(
            item_id=event_data["item_id"], deleter_user_id=event_data["deleter_user_id"]
        )
        async with AsyncSessionLocal() as session:
            service = WishlistService(session)
...
async def handle_wishlist_get_owner(event_data: dict):
    try:
        ...
        if not owner_id and event_data.get("telegram_id"):
