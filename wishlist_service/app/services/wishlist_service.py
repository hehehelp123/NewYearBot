...
class WishlistService:
    ...
    async def book_item(self, item_id: int, booker_user_id: int):
        ...
        failure_payload = {"item_id": item_id, "booker_user_id": booker_user_id, "telegram_id": booker_user_id}
        if not item:
        ...
        await kafka_producer.send("wishlist.item.booked", {
            "item_id": item_id, "item_name": item.name, "booked_by_user_id": booker_user_id,
            "owner_user_id": item.wishlist.owner_user_id, "telegram_id": booker_user_id,
            "is_infinitely_bookable": item.is_infinitely_bookable
        })

    async def unbook_item(self, item_id: int, unbooker_user_id: int):
        item = await self.get_item_by_id(item_id)
        failure_payload = {"item_id": item_id, "unbooker_user_id": unbooker_user_id, "telegram_id": unbooker_user_id}
        if not item:
        ...
        await kafka_producer.send("wishlist.item.unbooked", {
            "item_id": item_id, "item_name": item.name, "unbooked_by_user_id": unbooker_user_id,
            "owner_user_id": item.wishlist.owner_user_id, "telegram_id": unbooker_user_id,
            "is_infinitely_bookable": item.is_infinitely_bookable
        })

    async def delete_item(self, item_id: int, deleter_user_id: int):
        item = await self.get_item_by_id(item_id)
        failure_payload = {"item_id": item_id, "deleter_user_id": deleter_user_id, "telegram_id": deleter_user_id}
        if not item:
        ...
        try:
            ...
            await kafka_producer.send("wishlist.item.deleted", {
                "item_id": item_id, "item_name": item_name, "owner_user_id": owner_user_id,
    ...
    async def get_and_push_wishlist_for_owner(self, owner_user_id: int, requester_user_id: int):
        ...
        if not wishlist:
            ...
            await kafka_producer.send("wishlist.view.owner_failed", {
                "owner_user_id": owner_user_id, "telegram_id": requester_user_id,
    ...
    async def get_and_push_wishlist_for_viewer(self, owner_user_name: str, requester_user_id: int):
        ...
        if not wishlist:
            ...
            await kafka_producer.send("wishlist.view.viewer_failed", {
                "owner_user_name": owner_user_name, "telegram_id": requester_user_id,
                "reason": "Wishlist not found."
            })
            return
        try:
